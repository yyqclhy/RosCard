export class AiksWeatherCardEditor extends AiksControlBase {
  setConfig(config) {
    this._config = config || {};
    this.render();
  }
  ...
  render() {
    ...
    nameLabel.innerText = '天气名称:';

    const nameInput = document.createElement('input');
    nameInput.type = 'text';
    nameInput.value = this._config?.weather_name || '';
    nameInput.style.width = '100%';
    nameInput.style.padding = '8px';
    nameInput.style.boxSizing = 'border-box';
    nameInput.addEventListener('change', (e) => {
      this._config.weather_name = e.target.value;
    ...
    entityTitle.innerText = 'Weather Entities';
    ...
    this._config.entities.forEach((entity, index) => {
      ...
      select.style.flex = '1';
      select.innerHTML = '<option value="">Select Weather Entity</option>';
    ...
    const addBtn = this._createButton('Add Weather', () => {
