export class AiksCoverCardEditor extends AiksControlBase {
  ...
  render() {
    ...
    nameLabel.innerText = '窗帘名称:';

    const nameInput = document.createElement('input');
    nameInput.type = 'text';
    nameInput.value = this._config?.cover_name || '';
    nameInput.style.width = '100%';
    nameInput.style.padding = '8px';
    nameInput.style.boxSizing = 'border-box';
    nameInput.addEventListener('change', (e) => {
      this._config.cover_name = e.target.value;
    ...
    entityTitle.innerText = 'Cover Entities';
    ...
    this._config.entities.forEach((entity, index) => {
      ...
      select.style.flex = '1';
      select.innerHTML = '<option value="">Select Cover Entity</option>';
    ...
    const addBtn = this._createButton('Add Cover', () => {
