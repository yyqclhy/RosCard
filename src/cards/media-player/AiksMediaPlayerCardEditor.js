export class AiksMediaPlayerCardEditor extends AiksControlBase {
  ...
  render() {
    ...
    nameLabel.innerText = '播放器名称:';

    const nameInput = document.createElement('input');
    nameInput.type = 'text';
    nameInput.value = this._config?.media_name || '';
    nameInput.style.width = '100%';
    nameInput.style.padding = '8px';
    nameInput.style.boxSizing = 'border-box';
    nameInput.addEventListener('change', (e) => {
      this._config.media_name = e.target.value;
    ...
    entityTitle.innerText = 'Media Players';
    ...
    this._config.entities.forEach((entity, index) => {
      ...
      select.style.flex = '1';
      select.innerHTML = '<option value="">Select Media Player</option>';
    ...
    const addBtn = this._createButton('Add Media Player', () => {
