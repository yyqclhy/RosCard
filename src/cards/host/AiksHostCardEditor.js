export class AiksHostCardEditor extends AiksControlBase {
  setConfig(config) {
    this._config = config || {};
    this.render();
  }
  ...
  render() {
    ...
    nameLabel.innerText = '主机名称:';

    const nameInput = document.createElement('input');
    nameInput.type = 'text';
    nameInput.value = this._config?.host_name || '';
    nameInput.style.width = '100%';
    nameInput.style.padding = '8px';
    nameInput.style.boxSizing = 'border-box';
    nameInput.addEventListener('change', (e) => {
      this._config.host_name = e.target.value;
    ...
    entityTitle.innerText = 'Remote Entities';
    ...
    this._config.entities.forEach((entity, index) => {
      ...
      select.style.flex = '1';
      select.innerHTML = '<option value="">Select Remote Entity</option>';
    ...
    const addBtn = this._createButton('Add Host', () => {
      this._config.entities.push({ entity_id: '', commands: [] });
      this.dispatchEvent(new CustomEvent('config-changed', { detail: { config: this._config } }));
      this.render();
    });
